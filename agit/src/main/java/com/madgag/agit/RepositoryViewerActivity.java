...
public class RepositoryViewerActivity extends RepoScopedActivityBase {
	...
	private int readWriteFlag = READ_MODE;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		actionBar.addAction(new Action() {
			public void performAction(View view) {
				startService(new GitIntentBuilder("repo.SYNC").repository(
						repo()).toIntent());
			}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menu.add(0, DELETE_ID, 0, R.string.delete_repo_menu_option)
				.setShortcut('0', 'd').setIcon(ic_menu_delete);
		return true;
	}
	...
	private void doDeletion() {
		...
		gitAsyncTaskFactory.createTaskFor(new RepoDeleter(repo()),
				new CasualShortTermLifetime()).execute();
	}
	...
	BroadcastReceiver repoStateChangeBroadcastReceiver = new BroadcastReceiver() {
		public void onReceive(Context context, Intent intent) {
			Log.d(TAG, "repoStateChangeBroadcastReceiver got broadcast : "
					+ intent);
			if (!gitdir().exists()) {
				finish();
			}
		...
		case DELETION_CONFIRMATION_DIALOG:
			return new AlertDialog.Builder(this)
					...
					.setPositiveButton(button_yes,
							DELETION_CONFIRMATION_DIALOG_LISTENER)
					.setNegativeButton(button_no,
							DELETION_CONFIRMATION_DIALOG_LISTENER).create();
		default:
	...
	protected void onNewIntent(Intent newIntent) {
		Log.i(TAG, "onNewIntent called with " + newIntent + " "
				+ gitDirFrom(newIntent));
	}

	@Override
	protected void onResume() {
		super.onResume();
		registerReceiver(operationProgressBroadcastReceiver, new IntentFilter(
				"org.openintents.git.operation.progress.update"));

		registerReceiver(repoStateChangeBroadcastReceiver, new IntentFilter(
				actionWithSuffix(REPO_STATE_CHANGED_BROADCAST)));
	...
	public static PendingIntent manageRepoPendingIntent(File gitdir,
			Context context) {
		...
		return PendingIntent.getActivity(context, gitdir.hashCode(),
				intentForNotification, 0);
	}
